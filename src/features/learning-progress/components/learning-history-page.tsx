...
import { useLearningHistory, useLearningSession } from "../hooks";
...
export function LearningHistoryPage() {
  ...
  async function continueLearning() { const data = await session.mutateAsync(); router.push(`/learn/${data.session.attempt.id}`); }
  return <div className="mx-auto max-w-4xl space-y-7">
    ...
    {query.data.items.length === 0 ? <div className="rounded-2xl border bg-white dark:bg-neutral-900"><EmptyState icon={BookOpen} title="No lesson attempts yet" description="Your learning activity will appear here after your first session." /></div> : <div className="space-y-3">{query.data.items.map((item) => {
      ...
      return <article key={item.lessonAttemptId} className="flex flex-col gap-4 rounded-2xl border bg-white p-5 shadow-sm sm:flex-row sm:items-center dark:bg-neutral-900">
        ...
        {item.status === "Completed" ? <Button variant="outline" render={<Link href={`/lesson-attempts/${item.lessonAttemptId}/result`} />}>View result <ArrowRight /></Button> : <Button disabled={session.isPending} onClick={continueLearning}>{session.isPending ? <LoaderCircle className="animate-spin" /> : null}Continue <ArrowRight /></Button>}
      </article>;
    })}</div>}
    ...
    {session.isError && <p className="text-sm text-red-600" role="alert">We couldn’t continue your session. Please try again.</p>}
  </div>;
}
