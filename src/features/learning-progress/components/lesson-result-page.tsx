...
import { useLearningSession, useLessonAttemptResult } from "../hooks";
...
export function LessonResultPage({ attemptId }: { attemptId: string }) {
  ...
  async function continueLearning() {
    try { const data = await session.mutateAsync(); router.push(`/learn/${data.session.attempt.id}`); }
    catch (error) { if (axios.isAxiosError(error) && error.response?.status === 409) router.push("/progress"); }
  }
  return <div className="mx-auto max-w-3xl space-y-6">
    ...
    <div className="flex flex-col items-center"><Button size="lg" className="min-h-11 px-6" disabled={session.isPending} onClick={continueLearning}>{session.isPending ? <><LoaderCircle className="animate-spin" />Finding what’s next…</> : <>Continue Learning <ArrowRight /></>}</Button>{session.isError && <p className="mt-3 text-sm text-red-600" role="alert">We couldn’t find your next session. Please try again.</p>}</div>
  </div>;
}
